...
export default async function CorveePage() {
  ...
  const teamInCharge = await prisma.corveePlanning.findFirst({
    select: { team_letter: true, season: true },
    where: { year: currentYear, week: currentWeek },
    orderBy: { season: 'desc' },
  })

  const corveePlayers = teamInCharge
    ? await prisma.corveeTeams.findMany({
        where: {
          season: teamInCharge.season,
          team_letter: teamInCharge.team_letter,
        },
        include: { player: true },
      })
    : []
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-400">
            ...
            {teamInCharge ? ` • Seizoen ${teamInCharge.season}` : ''}
          </p>
        </div>
                  ...
                  {corveePlayers.map((entry) => (
                    ...
                    >
                      <div className="text-lg font-medium text-gray-900 dark:text-gray-100">
                        {CorveeService.getPlayerDisplayName(entry.player)}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="text-left p-4">
          <Link
            className="text-rose-500 dark:text-rose-500 hover:text-blue-600 dark:hover:text-blue-300"
            href="/corvee/teams"
          >
            Klik hier om de corvee teams en planning te zien
          </Link>
        </div>
      </div>
    </div>
